...
if (STYLE == "BYPASS") begin
  ...
  if (COALESCE_DISTANCE == 1 && POP_GARBAGE == 1) begin
    // Predicated pops are treated like coalesced pops so that
    // garbage doesn't accumulate. In case the original pop was
    // a depth of 1, DISTANCE_WIDTH ends up begin 0 which modelsim
    // does not allow as a replication factor
    assign pop_garbage = resetn;
  end else if(POP_GARBAGE == 0) begin
  ...
  //   I'm waiting for data from feedback (valid_in&~dir&~feedback_valid_in)
